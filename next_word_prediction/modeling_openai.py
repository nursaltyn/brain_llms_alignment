...
from utils_resp import get_resp, get_mask
import numpy as np
...
class Block(nn.Module):
    ...
    def forward(self, x, attention_mask=None, head_mask=None, output_attentions=False, 
        ...
        m = self.mlp(n)
        h = self.ln_2(n + m)
        print("dimension of h before:", h.shape)
        ...
        if to_distort == True:
            ...
            wt = data['weights']
            h = np.dot(brain_data, wt)
        print("dimension of h after:", h.shape)
        print("type of h after:", type(h))
        
        # outputs[0] = hidden states, outputs[1] = attentions
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(
        ...
        distort: Optional[int] = None,  # edit
        subject: Optional[str] = None,  # edit 
        area: Optional[str] = None,     # edit
        story: Optional[str] = None,    # edit
        mode: Optional[str] = None,     # edit
        ...
        distort = distort if distort is not None else None
        subject = subject if subject is not None else None
        area = area if area is not None else None
        story = story if story is not None else None
        ...
        if token_type_ids is not None:
            ...
            token_type_embeds = 0
        # prepare our initial input (that's why we have hidden states number 1 more than the number of layers!)
        ...
        if distort is not None:
            distort_layer = distort
            print("Some distortion will be added this time")
        for i, block in enumerate(self.h):
            print("Layer", i)
            if output_hidden_states:
                # we don't concatenate, just add them to the same tuple
                all_hidden_states = all_hidden_states + (hidden_states,)
            if distort and i == distort_layer:
                print("Distorting layer ", distort_layer)
                # args to block: hidden states (as an input), attent. mask, head mask
                outputs = block(hidden_states, attention_mask, head_mask[i], output_attentions=output_attentions, 
                                to_distort=True, layer_to_distort=distort_layer, subject=subject, 
                                area=area, story=story, mode=mode)
            else:
                outputs = block(hidden_states, attention_mask, head_mask[i], output_attentions=output_attentions)
