...
class LMFeatures():
    ...
    def make_stim(self, words, segment=True, context=None):
        """outputs matrix of features corresponding to the stimulus words
        """
        # edit: possibility to segment or not; control number of context words
        if context == None:
            context = self.context_words
        else:
            context = context
        context_array = self.model.get_story_array(words, context, segment=segment)
        # print("shape of the context array", context_array.shape)
        embs = self.model.get_hidden(context_array, layer = self.layer)
        # print("shape of the embs before stack", embs.shape)
        return np.vstack([embs[0, :context], 
            embs[:context_array.shape[0] - context, context]])