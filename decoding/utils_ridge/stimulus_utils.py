...
def load_textgrids(stories, data_dir: str):
    '''
    stories (list): a list of stories to consider for a subject (in the format "01", ... "09")
    '''
    base = join(data_dir, "train_stimulus")
    grids = {}
    for story in stories:
        grid_path = os.path.join(base, "story_%s.TextGrid" % story)
        grids[f"story_{story}"] = TextGrid(open(grid_path).read())

    return grids


def load_textgrids_eval(stories, data_dir: str):
    base = join(data_dir, "test_stimulus")
    grids = {}
    for story in stories:
        grid_path = os.path.join(base, "story_%s.TextGrid" % story)
        grids[f"story_{story}"] = TextGrid(open(grid_path).read())
